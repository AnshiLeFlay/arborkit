...
3. Tool guards and application approval run before dispatch when the agent bridge is used.
4. Type validation runs against a cloned proposed value.
5. The tree changes synchronously.
...
- `batchPatch` validates each live operation in order inside one such transaction.
