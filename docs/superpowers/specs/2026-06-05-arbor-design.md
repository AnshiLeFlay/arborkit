...
### Гранулярность
Не дробим до каждого скаляра. Дерево — **структурный скелет** (`object`/`array`-контейнеры); **content-bearing узлы** (`leaf` или явно помеченное непрозрачное поддерево) несут нагрузку и только они — кандидаты на эмбеддинг (как у LlamaIndex эмбеддятся только листья). Граница декомпозиции настраивается.
...
### Версионирование
- **event-log** — append-only, монотонный `seq`; источник правды истории.
- **`meta.version`** на узле — дешёвая проверка «изменилось ли».
- **снапшоты** — сериализация дерева (+вектора) в `StoragePort`; снапшот + последующие события = полное состояние; restore = снапшот + реплей.
- **`history(ref)`** читает события; реконструкция прошлого реплеем — опционально в v1.
...
2. **Мутации + версии:** `Mutator` (ops, валидация, scope, event-log), типизированные ошибки, `transaction`.
3. **Schema-optional:** реестр типов + Zod-адаптер; валидация на патче.
4. **Navigator:** `describe`/`get`/`find` + bounding/pagination/self-describing.
...
6. **Storage:** in-memory + file-snapshot адаптеры (дерево + вектора), restore.
7. **Тулсет:** `makeToolset` со scoping + LangChain-обёртки.
8. **Scenario/e2e + example.**
...
- Конкретный формат `pathPattern` для `find` (glob по JSON Pointer? подмножество JSONPath?).
- Дефолтная стратегия декомпозиции: что считается «непрозрачным поддеревом» (по типу? по размеру? по явной пометке?).
- Нужен ли `history`-реплей до произвольной версии в v1 или достаточно чтения лога.
- Выбор mock-эмбеддинга для тестов (хеш→вектор фиксированной размерности): какая размерность/функция, чтобы косинус был осмысленно различим.
